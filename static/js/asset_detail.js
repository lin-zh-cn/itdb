...
(function (jq) {
    ...
    function show_all_init() {
        $("#show_all_button").click(function (event) {
            var query_username = "all";
            var url = "/assets/asset_query_json/all/";
            show_asset_table(url)
        });

        //点击弹出框关闭按钮或'x'按钮时，取消查询按钮的data-toggle，data-target属性
        $("#off_modal,#x_x").bind('click', function () {
            $("#show_all_button").removeAttr("data-toggle");
            $("#show_all_button").removeAttr("data-target");
        });
